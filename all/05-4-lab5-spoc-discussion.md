...
### 总体介绍
 ...
 - 第一个用户进程创建有什么特殊的？
 > 答：第一个用户进程需要手动创建，先用内核进程创建一个内核进程的“壳”，并清空该进程里的内容，然后需要读入应用程序的信息，并把相应的页表等内容填入到该线程中，作为一个用户进程。

 - 系统调用的参数传递过程？
 > 答：在用户进程中，先将参数依次压入栈，然后将第一个参数的地址作为参数进行传递。

 - getpid的返回值放在什么地方了？
 > 答：getpid的值放在了用户态的堆栈中。

### 进程的内存布局

 - 尝试在进程运行过程中获取内核堆栈和用户堆栈的调用栈？
 > 答：在do_wait函数中添加相应的打印函数即可
