...
>  ucore OS配置过程包括首先在BIOS中完成一些最基本的初始化工作，然后加载并执行Bootloader，再从硬盘中读取kernel，最后将控制
权交给操作系统；驱动外设时钟主要包括利用pic_init完成中断信息的初始化，通过idt_init()初始化中断向量表，通过LIDT加载IDT，并要
初始化时钟。
...
int max(int n, ...) {            //n表示后面变参数量
    va_list ap;                  //定义一个va_list指针来访问参数表
    va_start(ap, n);             //初始化ap让它指向第一个变参
...
> 答：首先实现一个能输出字符串到控制台的bootloader，如显示“Hello world！”到屏幕上，方便后续的调试工作，要注意的是不同
的外设访问方式不同；接下来要实现一个可加载ELF格式文件的bootloader，并加载ELF文件，要注意检查ELF文件的有效性，同时bootloader
的大小要小于512字节。
...
> 答：如果想要获取一个系统调用的调用次数，可以在该系统调用的入口处统计,建立系统调用表来记录不同系统调用的调用次数；
      如果想获得所有系统调用的调用次数信息可以使用全局变量，每当需要调用系统调用时该变量就加1.
